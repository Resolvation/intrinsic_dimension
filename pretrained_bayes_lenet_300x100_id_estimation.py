...
def main():
    ...
    for d in range(50, 400, 50):
        print("Model with d = {:}".format(d))
        writer = SummaryWriter("./logs/Pretrained_Bayes_LeNet_300_100_ID_estimation/{:}".format(d))

        model = Bayes_LeNet_300_100_ID(d).to(device)
        ...
        save(model, "Pretrained_Bayes_LeNet_300_100_ID_estimation/{:}".format(d))
        writer.close()
        del model
